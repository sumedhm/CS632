...
long int *no_of_days_1_svc(date *dt, struct svc_req * rqstp){
    int day = dt->dd, month = dt->mm;
    long int year = dt->yy;
    int leap_days = *no_leap_years_1_svc(dt,rqstp);
...
int *get_day_1_svc(date *dt, struct svc_req * rqstp){
    long int days = *no_of_days_1_svc(dt,rqstp);
