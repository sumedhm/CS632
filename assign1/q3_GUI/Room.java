...
public class Room{
	...
	public int cancelBooking(int slot, int day, User user){
		...
		if(booked[day][slot].user.id==user.id) booked[day][slot] = null;
		else return 0;
		return 1;
	}
	...
	public String getBooking(int day, int slot){
		return new String("<br>Date - " + getDate(day) + ", Time - " + getTime(slot) + "<br>Room - " + this.name + ", " + this.location +
						 	"<br>By - " + booked[day][slot].user.name + "<br>Event: " + booked[day][slot].event +
							"<br>Event Description: " + booked[day][slot].description + "<br>");
	}
